...
try:
    ...
    def log(message, level=LOG_INFO, throttle=0, suppress_repeat=True):
        if level >= LOG_INFO:  # 只打印 INFO 及以上
            print(message)

class SMERMemory:
    """结构化掩码经验回放内存
    
    用于存储和采样包含主机和从机信息的结构化经验数据。
    支持动态数量的从机，并在采样时输出填充好的批次张量和对应的掩码。
    """
    ...
    def __init__(self, capacity: int, obs_dims: Dict[str, int], action_dims: Dict[str, int], device: torch.device):
        ...
        Args:
            capacity: 内存容量
            obs_dims: 观测维度字典，格式为 {"leader": dim, "followers": dim}
            action_dims: 动作维度字典，格式为 {"leader": dim, "followers": dim}
            device: 存储张量的设备
        """
        ...
        log(f"SMERMemory 初始化完成: 容量={capacity}, 观测维度={obs_dims}, 动作维度={action_dims}", LOG_INFO)
    ...
    def store_transition(self, observation: Dict, action: Dict, reward: Dict, 
        ...
        Args:
            observation: 观测字典 {"leader": obs_leader, "followers": [obs_f1, obs_f2, ...]}
            action: 动作字典 {"leader": act_leader, "followers": [act_f1, act_f2, ...]}
            reward: 奖励字典 {"leader": rew_leader, "followers": [rew_f1, rew_f2, ...]}
            next_observation: 下一观测字典 {"leader": next_obs_leader, "followers": [next_obs_f1, ...]}
            done: 终止标志

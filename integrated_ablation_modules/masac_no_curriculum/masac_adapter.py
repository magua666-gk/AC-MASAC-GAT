...
# 导入新创建的类
from integrated_ablation_modules.masac_no_curriculum.agent_pool import AgentPool, DynamicActor
...
class TypeAwareActor:
    ...
    def evaluate(self, s, create_graph=True):
        ...
        # 使用更稳健的数值稳定性计算
        log_prob = dist.log_prob(z) - torch.log(torch.clamp(1 - action.pow(2), min=1e-4, max=1.0))
...
class MASACEntroy():
    def __init__(self, action_dim=2):
        ...
        # 创建可学习的log_alpha参数
        self.log_alpha = torch.log(torch.ones(1) * self.initial_alpha).requires_grad_(True)
    ...
    def to(self, device):
        ...
        for entroy in self.entroys:
            if hasattr(entroy, 'log_alpha') and torch.is_tensor(entroy.log_alpha):
                entroy.log_alpha = entroy.log_alpha.to(device)
            if hasattr(entroy, 'alpha') and torch.is_tensor(entroy.alpha):
                entroy.alpha = entroy.alpha.to(device)

...
# Try to import project logging tools
try:
    from integrated_ablation_modules.masac_no_curriculum.masac_adapter import log, LOG_INFO, LOG_WARNING, LOG_ERROR, LOG_DEBUG
except ImportError:
    # Simple fallback logging implementation
    LOG_INFO, LOG_WARNING, LOG_ERROR, LOG_DEBUG = 1, 2, 3, 4
    def log(message, level=LOG_INFO, throttle=0, suppress_repeat=True):
        if level >= LOG_INFO:  # Only print INFO and above
            print(message)

class SMERMemory:
    """Structured Masked Experience Replay Memory
    
    Stores and samples structured experience data with leader and follower information.
    Supports dynamic number of followers and outputs padded batch tensors with masks.
    """
    ...
    def __init__(self, capacity: int, obs_dims: Dict[str, int], action_dims: Dict[str, int], device: torch.device):
        ...
        Args:
            ...
            device: Device for storing tensors
        """
    ...
    def store_transition(self, observation: Dict, action: Dict, reward: Dict, 
        ...
        """Store a single transition to replay buffer
        ...
        Args:
            ...
            stage_tag: Stage tag for marking curriculum stage
        """
        ...
        # Normalize rewards to unified dimension (1)
        ...
        experience = {
            ...
            "done": done,
            "stage_tag": stage_tag  # New field: stage tag
        }
        
        # Add to buffer
        self.buffer.append(experience)
        
        # Update counter - use actual buffer length
        self.memory_counter = len(self.buffer)
        
        # Log
        if self.memory_counter % 1000 == 0 and self.memory_counter > 0:
            log(f"SMERMemory: Stored {self.memory_counter} experiences (capacity: {self.capacity})", LOG_DEBUG)

    def _validate_and_convert_data(self, data_dict: Dict, expected_dims_dict: Dict) -> Dict:
        """Validate and convert input data types and shapes
        
        Args:
            data_dict: Input data dictionary
            expected_dims_dict: Expected dimension dictionary (obs_dims or action_dims)
